...
function usersFilter(nsrs) {
  ...
  if (nsrs != "undefined" && nsrs != undefined && nsrs.length > 0) {
    ...
    for (var i = 1; i < nsrs.length; i++) {
      var nsrs_keys = [nsrs[i].shxydm, nsrs[i].gsnsrsbh, nsrs[i].dsnsrsbh];
      var nsrs_yhsfdm = Number(nsrs[i].yhsfdm);
      var ifexit = false;

      for (var j = 0; j < temp_nsrs.length; j++) {
        var temp_nsrs_keys = [temp_nsrs[j].shxydm, temp_nsrs[j].gsnsrsbh, temp_nsrs[j].dsnsrsbh];
...
function ifEqual(arr1, arr2) {
  ...
  for (var i = 0; i < arr1.length; i++) {
    ...
    if (arr1[i] != "undefined" && arr1[i] != undefined && arr1[i] != "" && arr2.indexOf(arr1[i]) > -1) {
      flag = true;
      break;
    }
  }
